...
public class HologramPacketListener extends PacketAdapter {
    ...
    static {
        ...
        ENTITY_USE_PACKET_CLASS = nmsClassResolver.resolveWrapper("PacketPlayInUseEntity", "net.minecraft.network.protocol.game.PacketPlayInUseEntity");
        PACKET_DATA_SERIALIZER_CLASS = nmsClassResolver.resolveWrapper("PacketDataSerializer", "net.minecraft.network.PacketDataSerializer");
        ENTITY_USE_PACKET_ID_FIELD = new FieldResolver(ENTITY_USE_PACKET_CLASS.getClazz()).resolveAccessor("a");
        PACKET_DATA_SERIALIZER_CONSTRUCTOR = new ConstructorResolver(PACKET_DATA_SERIALIZER_CLASS.getClazz()).resolveWrapper(new Class[]{ByteBuf.class});
        PACKET_DATA_SERIALIZER_READ_INT_METHOD = new MethodResolver(PACKET_DATA_SERIALIZER_CLASS.getClazz()).resolveWrapper("e");
        ENTITY_USE_PACKET_A_METHOD = new MethodResolver(ENTITY_USE_PACKET_CLASS.getClazz()).resolveWrapper(
                ResolverQuery.builder().with("b", PACKET_DATA_SERIALIZER_CLASS.getClazz()).build());
    }

    private static final Cache<UUID, Integer> COOLDOWN = CacheBuilder.newBuilder().expireAfterWrite(20, TimeUnit.MILLISECONDS).build();

    @Override
    public void onReceiving(PacketEvent event) {
        if (event.getPacket() != null && event.getPacket().getClass().isAssignableFrom(ENTITY_USE_PACKET_CLASS.getClazz())) {
            int entityId = ENTITY_USE_PACKET_ID_FIELD.get(event.getPacket());
            if (COOLDOWN.asMap().containsKey(event.getPlayer().getUniqueId()) && COOLDOWN.asMap().getOrDefault(event.getPlayer().getUniqueId(), -1) == entityId)
                return;
            ...
            for (Hologram hologram : HologramManager.get().getHolograms().values()) {
                if (!hologram.getLocation().getWorld().getName().equals(event.getPlayer().getLocation().getWorld().getName()))
                    continue;
    ...
    protected int getEntityUseActionOrdinal(Object packet) {
        if (packet != null && packet.getClass().isAssignableFrom(ENTITY_USE_PACKET_CLASS.getClazz())) {
